Privacy Policy
Effective Date: January 1, 2026

This APP ("we", "our", or "us") respects your privacy. This Privacy Policy explains how we collect, use, and protect your information when you use the mobile application ("App").

1. Information We Collect

a) Information You Provide
- Questionnaire responses
- Relationship-related inputs
- Optional account information (if applicable)
- Email address (if provided for support or account creation)

b) Automatically Collected Information
- Device type
- App usage data
- Anonymous analytics data

We do NOT collect:
- Biometric data
- Location tracking (unless explicitly enabled)
- Private messages outside the App

2. How We Use Information

We use your information to:
- Generate relationship analysis reports
- Improve app performance and user experience
- Provide customer support
- Prevent fraud or abuse

3. Data Storage

Questionnaire data may be processed locally on your device or stored securely on encrypted servers (if cloud processing is used).

We implement industry-standard security measures to protect your data.

4. Data Sharing

We do NOT sell your personal data.

We may share limited information with:
- Payment processors (e.g., Stripe)
- Analytics providers (e.g., Firebase)
- Legal authorities if required by law

5. Data Retention

We retain data only as long as necessary to provide services or comply with legal obligations.

Users may request deletion of their data by contacting:
krizza lynne rivera de mesa

6. Children's Privacy

App is not intended for users under 18 years of age.

7. Changes to This Policy

We may update this Privacy Policy from time to time. Continued use of the App constitutes acceptance of the updated policy.

8. Contact

If you have questions about this Privacy Policy, contact:
krizza lynne reivera de mesa
buzzbreakphilippines@gmail.com
